...
def server(frozen):
    env = dict(
        ...
        POSTGRES_DB="roomreserve_manual",
    ...
    p = subprocess.Popen(
        [
            os.path.expanduser("~/.virtualenvs/roomreserve/bin/python"),
            "manage.py",
            "runserver",
            f"127.0.0.1:{PORT}",
            "--noreload",
...
DESKTOP = {"viewport": {"width": 1280, "height": 860}, "device_scale_factor": 2, "locale": "th-TH"}
...
def ctx(browser, kind, user=None):
    ...
    if user:
        ...
        pg.click("form button[type=submit] >> nth=-1") if False else pg.locator(
...
def shot(pg, path, name, full=False, cap=None):
    ...
    bottom = pg.evaluate(
        "() => { const m = document.querySelector('main'); const r = m.getBoundingClientRect(); return Math.ceil(r.bottom + window.scrollY + 8); }"
    )
    cap = cap or (1500 if width < 500 else 1100)
    height = min(bottom, cap)
    pg.screenshot(
        path=f"{OUT}/{name}.png", full_page=True, clip={"x": 0, "y": 0, "width": width, "height": height}
    )
...
def session_a(b):
    ...
    c.close()
    for user, room, name in [
        ("660000002", 2, "m_door_not_open"),
        ("660000003", 13, "m_door_pending"),
        ("660000004", 4, "m_door_other_room"),
        ("660000011", 9, "m_door_walkin"),
    ]:
        c, pg = ctx(b, "m", user)
        shot(pg, f"/th/r/{room}/", name)
        c.close()
...
def session_b(b):
    c, pg = ctx(b, "m", "660000005")
    shot(pg, "/th/r/5/", "m_door_closed")
    c.close()
