...
admin, _ = User.objects.get_or_create(
    username="maintainer", defaults={"email": "admin@example.invalid", "name_th": "ผู้ดูแลระบบ"}
)
...
print(
    json.dumps(
        {"students": [u.username for u in s[:9]], "rooms": {r.number: r.pk for r in Room.objects.all()}}
    )
)
